...
try {
    ...
    $stmtTransaksiTerbaru = $conn->prepare("SELECT t.date, t.type, t.description, t.amount, p.name as product_name, t.quantity FROM transactions t LEFT JOIN products p ON t.product_id = p.id ORDER BY t.date DESC LIMIT :limit OFFSET :offset");
    $stmtTransaksiTerbaru->bindValue(':limit', $limit, PDO::PARAM_INT);
    $stmtTransaksiTerbaru->bindValue(':offset', $offset, PDO::PARAM_INT);
    $stmtTransaksiTerbaru->execute();
    $transaksiTerbaru = $stmtTransaksiTerbaru->fetchAll();

    // --- Data untuk Grafik Tren (Penjualan dan Pengeluaran Bulanan) ---
    // Mengambil data 6 bulan terakhir
    // Menggunakan DATE_FORMAT('%Y-%m') untuk pengurutan yang lebih robust
    $stmtMonthlySales = $conn->query("
        ...
        LIMIT 6
    ");
    $rawMonthlySales = $stmtMonthlySales->fetchAll(PDO::FETCH_GROUP|PDO::FETCH_ASSOC);
    ...
    $stmtMonthlyExpenses = $conn->query("
        ...
        LIMIT 6
    ");
    $rawMonthlyExpenses = $stmtMonthlyExpenses->fetchAll(PDO::FETCH_GROUP|PDO::FETCH_ASSOC);
    ...
    for ($i = 5; $i >= 0; $i--) {
        ...
        $monthlySales[] = isset($rawMonthlySales[$period]) ? $rawMonthlySales[$period][0]['total_amount'] : 0;
        $monthlyExpenses[] = isset($rawMonthlyExpenses[$period]) ? $rawMonthlyExpenses[$period][0]['total_amount'] : 0;
    }
    ...
    // Anda bisa menampilkan pesan error yang lebih user-friendly jika diperlukan
    // $error_message = "Terjadi kesalahan saat memuat data dashboard: " . $e->getMessage();
    $transaksiTerbaru = []; // Pastikan variabel tetap diinisialisasi
    $products = []; // Pastikan ini juga kosong jika ada error
}
